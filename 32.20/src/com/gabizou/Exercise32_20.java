...
public class Exercise32_20 extends JApplet {
	/**
	 * 
	 */
	...
	public static void main(String[] args) {
		JFrame frame = new JFrame("Sudoku Search Animation");
		JApplet applet = new Exercise32_20();
		frame.add(applet);
	...
	public boolean search(int[][] grid) {
		...
		while (!found && k < freeCellList.length) {
			...
			} else { // grid[i][j] is 9, go back
				while (grid[i][j] == 9) {
					grid[i][j] = 0; // Reset to empty cell
					cells[i][j].setText(null);
					if (k == 0) {
	...
	public void setANewValue(int[][] grid, int i1, int j1) {
		...
		cells[i1][j1].setText(grid[i1][j1] + "");
		//displayGrid(grid);   
