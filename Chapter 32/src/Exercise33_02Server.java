...
public class Exercise33_02Server extends JFrame {
	/**
	 * 
	 */
	...
	class ThreadHandler extends Thread {
		...
		public void run() {
			try {
				...
				Scanner isFromClient = new Scanner(new InputStreamReader(
						connectToClient.getInputStream()));
				PrintWriter osToClient = new PrintWriter(connectToClient.getOutputStream(), true);

				// Continuously serve the client
				while (true) {
					// Get radius from the client
					double radius = isFromClient.nextDouble();
