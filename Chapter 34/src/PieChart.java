...
public class PieChart extends JPanel implements ActionListener {
	...
	Color[] colors = { Color.red, Color.yellow, Color.green, Color.blue, Color.cyan, Color.magenta,
			Color.orange, Color.pink, Color.darkGray };
		...
		for (int i = 0; i < data.length; i++) {
			...
			g.drawString(
					...
					(int) (getWidth() / 2 + radius
							* Math.cos((angle1 + angle2 / 2) * 2 * Math.PI / 360)),
					(int) (getHeight() / 2 - radius
							* Math.sin((angle1 + angle2 / 2) * 2 * Math.PI / 360)));
		}
	}
