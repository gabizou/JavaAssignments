...
public class Exercise30_20 extends JFrame {
	private List<Vertex> vertices = new ArrayList<Vertex>();
	private List<AbstractGraph.Edge> edges = new ArrayList<AbstractGraph.Edge>();

	public Exercise30_20() {
		readGraph();
		...
		try {
			...
			for (int i = 0; i < n; i++) {
				...
				for (int k = 3; k < tokens.length; k++)
					edges.add(new AbstractGraph.Edge(i, Integer.parseInt(tokens[k].trim())));
			}
	...
	public static void main(String[] args) {
		JFrame frame = new Exercise30_20();
		frame.setTitle("Display a graph");
