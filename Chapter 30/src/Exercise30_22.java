...
public class Exercise30_22 extends JApplet {
	// Circles are stored in a list
	private List<Circle> circles = new ArrayList<Circle>();

	public Exercise30_22() {
	...
	class CirclePanel extends JPanel {
				...
				public void mouseDragged(MouseEvent e) {
					...
					for (; i < circles.size(); i++)
						if (circles.get(i).contains(e.getPoint())) {
							circles.get(i).x = e.getX();
							circles.get(i).y = e.getY();
							break;
						}
					repaint();
				}
		...
		protected void paintComponent(Graphics g) {
			...
			boolean isAllCirclesConnected = circles.size() == tree.getNumberOfVerticesFound();
	...
	public static void main(String[] args) {
		...
		JApplet applet = new Exercise30_22();
		frame.add(applet);
		frame.setTitle("Exercise29_22");
